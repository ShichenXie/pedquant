...
geted_nbs_symbol1 = function(geo_type=NULL, frequency=NULL, symbol='zb', eng=FALSE) {
  dim_geo_type = dim_frequency = dim_region = dim_sta_db = . = id = name = isParent = pid = NULL
  ...
  # frequency
  frequency = check_nbs_frequency(frequency, geo_type)
  # name of geography in NBS
  nbs_geo = df_nbs_geo()[
    dim_region=='cn' & dim_geo_type==geo_type & dim_frequency==frequency , dim_sta_db]
...
#' @export
geted_nbs_symbol = function(geo_type=NULL, frequency=NULL, eng=FALSE) {
  ...
  # type in selected symbol
    ...
    is_parent = TRUE
    while (is_parent) {
      symbol_df = geted_nbs_symbol1(geo_type, frequency, sel_symbol, eng)
...
date_to_mqa = function(from, to, frequency) {
...
geted_nbs_querydat = function(nbs_geo, symbol, region=NULL, from, eng=FALSE) {
  ...
  query_list = list(
    ...
    colcode='sj',
    wds=wds,
    dfwds=paste0('[{"wdcode":"zb","valuecode":"',symbol,'"},{"wdcode":"sj","valuecode":"',sj_value,'"}]'),
    k1=time_sec
  )
  req = GET(url_nbs, query=query_list)
  check_http_status_nbs(req)
...
nbs_jsondat_format = function(jsondat) {
  ...
  code_names2 = sub('sj','date',sub('reg','region',sub('zb','symbol',code_names)))
  # code dataframe
  ...
  zb_df = setDT(jsondat$returndata$wdnodes$nodes[[which('zb' == code_names)]])[,.(unit, symbol_name=cname, symbol=code)]
  dat2 = cbind(
  ...
  if ('reg' %in% code_names) {
    reg_df = setDT(jsondat$returndata$wdnodes$nodes[[which('reg' == code_names)]])[,.(region_name=cname, region=code)]
    
    dat2 = dat2[reg_df, on='region']
  }
...
#' 
#' # specify paratmeters
#' dt1 = geted_nbs(geo_type='national', frequency='quarterly', 
#'   symbol='A010101')
#' 
#' # or using 'n'/'q' represents 'national'/'quarterly'
#' dt2 = geted_nbs(geo_type='n', frequency='q', symbol='A010101')
#' 
#' 
...
geted_nbs = function(geo_type=NULL, frequency=NULL, symbol=NULL, region=NULL, from='2010-01-01', to=Sys.Date(), na_rm=FALSE, eng=FALSE) {
  ...
  to = check_fromto(to)
  fromto = date_to_mqa(from, to, frequency)
