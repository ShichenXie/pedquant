...
pq1_trend = function(dt, date_range='max', from=NULL, to=Sys.Date(), x='close|value', base_value=1) {
...
# 
# \code{pq_trend} provides an easy way to create the performance trends for a set of time series data.
# 
...
# 
# @export
pq_trend = function(dt, date_range='max', from=NULL, to=Sys.Date(), x='close|value', base_value=1) {
    symbol = NULL
    ...
    for (s in sybs) {
        ...
        dt_list[[s]] = do.call(pq1_trend, args = list(dt=dt_s, x=x, date_range=date_range, from=from, to=to, base_value=base_value))
    }
